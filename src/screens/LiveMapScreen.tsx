...
export default function LiveMapScreen({ route, navigation }: any) {
  ...
  const displayedVehicles = useMemo(() => targetVehicle ? [targetVehicle] : [], [targetVehicle]);
  ...
  // Animation Listeners
  useEffect(() => {
    const id = animatedValue.addListener(({ value }) => { currentValue.current = value; });
  ...
  const addressContainerHeight = animatedValue.interpolate({ inputRange: [0, 1], outputRange: [20, 70], extrapolate: 'clamp' });
  const collapsedOpacity = animatedValue.interpolate({ inputRange: [0, 0.2], outputRange: [1, 0], extrapolate: 'clamp' });
  const expandedOpacity = animatedValue.interpolate({ inputRange: [0.1, 0.4], outputRange: [0, 1], extrapolate: 'clamp' });
            ...
            function handleMessage(payload) {
                try {
                    ...
                    if (msg.type === 'RECENTER') {
                        map.flyTo({ center: [msg.payload.lng, msg.payload.lat], zoom: 16, speed: 1.5, curve: 1 });
                        return;
                    }
                    ...
                    vehicles.forEach(function(v) {
                        ...
                        if (markers[v.id]) {
                            ...
                            var dist = Math.sqrt(Math.pow(diffLat, 2) + Math.pow(diffLng, 2));

                            var POS_THRESHOLD = 0.00002; // ~2m
                            var ROT_THRESHOLD = 0.00015; // ~15m

                            if (dist > POS_THRESHOLD) {
                                var bearing;
                                if (dist > ROT_THRESHOLD) {
                                    bearing = getBearing(startLngLat[1], startLngLat[0], targetLngLat[1], targetLngLat[0]);
                                } else {
                                    bearing = vehicleRotations[v.id] || 0;
                                }
                            ...
                            markers[v.id] = marker;
                            vehicleRotations[v.id] = 0; 
                            if (v.id === targetId && pathHistory.length === 0) pathHistory.push(targetLngLat);
                        }
                    });
                ...
                function loop(now) {
                    ...
                    if (progress < 1) {
                        ...
                        if (markers[id]) {
                            ...
                            vehicleRotations[id] = targetBearing;
                        }
                    }
                }
                if (animState[id]) cancelAnimationFrame(animState[id].requestID);
  ...
  const onWebViewMessage = (event: any) => {
      try {
          ...
          if (data.type === 'ZOOM_LIMIT_REACHED') showToast("Maximum zoom level reached");
      } catch { }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.fabWrapper, buttonStyle]}>
          <TouchableOpacity style={[styles.fab, styles.recenterFab]} onPress={handleRecenter} activeOpacity={0.8}>
                ...
                <>
                    <View style={styles.sheetHeader}>
                        <View style={styles.textContainer}>
                            ...
                            <Animated.View style={[styles.addressRow, { height: addressContainerHeight }]}>
                                <Animated.Text style={[styles.addressText, styles.absoluteText, { opacity: collapsedOpacity }]} numberOfLines={1}>{address}</Animated.Text>
                                <Animated.Text style={[styles.addressText, styles.absoluteText, { opacity: expandedOpacity }]}>{address}</Animated.Text>
                            </Animated.View>
                        </View>
                    ...
                    <View style={styles.statsRow}>
                        ...
                        <View style={styles.statItem}>
                            ...
                            <Text style={styles.statValue}>{new Date(targetVehicle.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</Text>
                        </View>
                    </View>
                </>
            ) : (
...
const styles = StyleSheet.create({
  ...
  map: { flex: 1 },
  toastContainer: { position: 'absolute', alignSelf: 'center', backgroundColor: 'rgba(0,0,0,0.8)', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 25, zIndex: 100 },
  toastText: { color: 'white', fontWeight: '600', fontSize: 14 },
  fabWrapper: { position: 'absolute', right: 20, zIndex: 20 },
  fab: { width: 44, height: 44, backgroundColor: 'white', borderRadius: 22, alignItems: 'center', justifyContent: 'center', ...SHADOWS.medium, elevation: 5 },
  recenterFab: { backgroundColor: COLORS.primary },
  fabIcon: { fontSize: 22, color: COLORS.textPrimary }, 
  bottomSheet: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: 'white', borderTopLeftRadius: 30, borderTopRightRadius: 30, ...SHADOWS.medium, elevation: 25, overflow: 'hidden' },
  dragHandleArea: { height: 30, alignItems: 'center', justifyContent: 'center', backgroundColor: 'white' },
  dragHandle: { width: 40, height: 5, backgroundColor: '#E0E0E0', borderRadius: 10 },
  ...
  speedBadge: { alignItems: 'center', justifyContent: 'center', backgroundColor: '#eff6ff', paddingVertical: 10, paddingHorizontal: 16, borderRadius: 16, borderWidth: 1, borderColor: '#dbeafe' },
